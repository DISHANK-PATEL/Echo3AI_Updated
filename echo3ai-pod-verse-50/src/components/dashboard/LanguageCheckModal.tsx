...
const LanguageCheckModal: React.FC<LanguageCheckModalProps> = ({ isOpen, onClose, podcast }) => {
  ...
  const performLanguageCheck = async () => {
    ...
    try {
      ...
      const transcriptResponse = await fetch(`http://localhost:5001/api/podcasts/${podcast._id}/transcript`);
      ...
      const analysisResponse = await fetch('http://localhost:5001/api/language-check', {
        method: 'POST',
        headers: {
